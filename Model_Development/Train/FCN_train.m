function FCN=FCN_train(x_i,x_q,y_i,y_q,M)
% 设置输入/输出矩阵，并设计网络
dim = length(x_i);
X = zeros(2*(M + 1), dim-M);
...
layers = [
    ...
    fullyConnectedLayer(32,"Name","fc_8")
    reluLayer("Name","relu_8")
    fullyConnectedLayer(16,"Name","fc_9")
    fullyConnectedLayer(8,"Name","fc_10")
    fullyConnectedLayer(2,"Name","fc_11")
    regressionLayer("Name","regressionoutput")];%-47dB
...
FCN = trainNetwork(X(:, 1:num), Y(:, 1:num), layers, options);
